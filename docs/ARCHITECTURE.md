...
5. Rust command가 선택 파일 존재 여부, 지원 확장자, 대상 경로 중복, 기존 파일 충돌, 애니메이션 WebP와 HEIC sequence 여부를 검사한다.
