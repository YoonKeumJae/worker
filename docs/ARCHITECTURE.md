...
PNG/SVG 기본 파일명 생성은 `src/tools/qr-code/qrCodeFileName.ts`에 둔다. 이 helper는 정규화된 URL 전체를 소문자 slug로 만들고 OS 파일명에 안전하지 않은 문자와 파일명 제어문자를 `-`로 치환한다.
...
7. UI가 `qrCodeFileName.ts`로 정규화된 URL 전체 기반 `{URL slug}-qr.png` 기본 파일명을 만들고 `save_qr_code_png` command에 경로와 bytes를 전달한다.
...
11. UI가 `qrCodeFileName.ts`로 정규화된 URL 전체 기반 `{URL slug}-qr.svg` 기본 파일명을 만들고 `save_qr_code_svg` command에 경로와 SVG text를 전달한다.
