...
- QR 기능은 후속 PR에서 구현되어 현재 `docs/FEATURES/qr-code.md` 기준으로 관리한다.
- 현재 검증 명령은 `README.md`와 `docs/DEVELOPMENT.md`를 기준으로 한다.
