...
- URL 입력 아래에는 내보내기 배경 옵션 `흰색`, `투명`을 둔다.
- 결과 action은 `PNG 저장`, `SVG 저장`, `이미지 복사`로 묶는다.
...
- 기본값은 `흰색`
- `투명` 옵션은 PNG/SVG/이미지 복사에만 적용
- 미리보기 surface는 스캔 안정성을 위해 흰색 고대비 배경 유지
- 투명 배경 판독성 안내 문구는 짧게 표시
